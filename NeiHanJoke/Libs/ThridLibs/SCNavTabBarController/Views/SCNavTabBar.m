...
- (NSArray *)itemsWidth{
    if (!_itemsWidth) {
        _itemsWidth = [[NSArray alloc] init];
    }
    return _itemsWidth;
}
...
- (CGFloat)popMenuHeight
{
    ...
    for (NSInteger index = 0; index < self.itemsWidth.count; index++)
    {
        buttonX += [self.itemsWidth[index] floatValue];
        
        @try {
#warning 原代码为if ((buttonX + [_itemsWidth[index + 1] floatValue]) >= SCREEN_WIDTH)
            if ((buttonX + [self.itemsWidth[index] floatValue]) >= SCREEN_WIDTH)
            {
...
- (void)popItemMenu:(BOOL)pop
{
    ...
    else
    {
        [UIView animateWithDuration:0.5f animations:^{
            ...
            [self viewShowShadow:_arrowButton shadowRadius:10.0f shadowOpacity:5.0f];
        }];
    }
}
...
- (void)setCurrentItemIndex:(NSInteger)currentItemIndex
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        _line.frame = CGRectMake(button.frame.origin.x + 2.0f, _line.frame.origin.y, [self.itemsWidth[currentItemIndex] floatValue] - 4.0f, _line.frame.size.height);
    }];
}

- (void)updateData{
    _arrowButton.backgroundColor = self.backgroundColor;
    self.itemsWidth = [self getButtonsWidthWithTitles:_itemTitles];
    if (self.itemsWidth.count)
    {
