...
-(float)moveProgressControllWithTempPoint:(CGPoint)tempPoint{
    //90代表整个屏幕代表的时间
    float tempValue = _touchBeginValue + TotalScreenTime * ((tempPoint.x - _touchBeginPoint.x)/kXXYScreenW);
...
-(void)hideTheLightViewWithHidden:(BOOL)hidden{
    if (self.isFullscreen) {//全屏才出亮度调节的view
        ...
        if ([UIDevice currentDevice].systemVersion.floatValue >= 8.0) {
            self.effectView.frame = CGRectMake(kXXYScreenH/2-155/2, kXXYScreenW/2-155/2, 155, 155);
        }else{
            self.lightView.frame = CGRectMake(kXXYScreenH/2-155/2, kXXYScreenW/2-155/2, 155, 155);
        }
    }else{
        return;
    }
